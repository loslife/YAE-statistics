app.controller('dakaPlayCtrl', ['$rootScope', '$scope', '$http', function ($rootScope, $scope, $http) {
    ...
    $scope.$watch('CateParams', function(newVal, oldVal){
        if (newVal !== oldVal &&
            (newVal.cateId !== oldVal.cateId || newVal.order !== oldVal.order || newVal.num !== oldVal.num) && newVal) {
            getCatePlayData(newVal.cateId, newVal.order, 10);
        }
        if(newVal !== oldVal && newVal.showSpline !== oldVal.showSpline){
            resetRefresh();
        }
    }, true);
    ...
    function getCatePlayData(id, order, num){
        if(playDataCache[id + "_" + order]){
            $scope.data = playDataCache[id + "_" + order];
            return;
        }
