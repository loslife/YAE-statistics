...
angular.module('app.controllers', ['pascalprecht.translate', 'ngCookies'])
  ...
  .controller('SigninFormController', ['$scope', '$http', '$state', function($scope, $http, $state) {
    ...
    $scope.login = function() {
      $scope.authError = null;
      // Try to login
      $http.post('/svc/pageAjaxLogin', {username: $scope.user.username, password: $scope.user.password})
      .then(function(response) {
        if (response.data.code || response.data.errorCode) {
          $scope.authError = '用户名或密码错误';
        }
        else if(response.status != 200){
            $scope.authError = '系统忙，请稍后再试！';
        }
        else{
          $state.go('app.mkt-trace');
        }
      }, function(x) {
