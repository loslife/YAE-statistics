...
(function(window, $) {
    ...
    var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse) {
        ...
        self.configureColumnWidths = function() {
            ...
            if (asterisksArray.length > 0) {
                ...
                var remainigWidth = self.rootDim.outerWidth - (totalWidth+1);
    ...
    var ngRow = function (entity, config, selectionProvider, rowIndex, $utils) {
        ...
        self.toggleSelected = function(event) {
            ...
            if(self.selectionProvider.lastClickedRow){
                self.selectionProvider.lastClickedRow.elm.removeClass("selected");
            }
            self.elm.addClass("selected");
    ...
    var ngStyleProvider = function($scope, grid) {
        ...
        $scope.topPanelStyle = function() {
            return { "width": grid.rootDim.outerWidth + "px", "height": $scope.topPanelHeight() + "px" ,"line-height": $scope.topPanelHeight()-10 + "px" };
        };
        $scope.headerStyle = function() {
            return { "width": (grid.rootDim.outerWidth-1) + "px", "height": grid.config.headerRowHeight + "px" };
        };
        $scope.groupPanelStyle = function () {
            return { "width": (grid.rootDim.outerWidth+1) + "px", "height": "48px" };
        };
            ...
            compile: function() {
                return {
                    pre: function($scope, iElement) {
                        ...
                        if($scope.enableCellSelection && cellElement[0].className && cellElement[0].className.indexOf('ngSelectionCell') == -1){
    ...
    angular.module("ngGrid").run(["$templateCache", function($templateCache) {
        ...
        $templateCache.put("footerTemplate.html",
            "<div ng-show=\"showFooter\" class=\"ngFooterPanel\" ng-class=\"{'ui-widget-content': jqueryUITheme, 'ui-corner-bottom': jqueryUITheme}\" ng-style=\"footerStyle()\">" +
                "    <div class=\"ngPagerContainer\" style=\"float: right; margin-top: 4px;\" ng-show=\"enablePaging\" ng-class=\"{'ngNoMultiSelect': !multiSelect}\">" +
        ...
        $templateCache.put("headerRowTemplate.html",
            "<div style=\"height:{{ col.headerRowHeight }};line-height: {{col.headerRowHeight-10}}px\" ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngHeaderCell\" ng-header-cell></div>"
        );
        ...
        $templateCache.put("menuTemplate.html",
            "<div ng-show=\"showColumnMenu || showFilter\"  class=\"ngHeaderButton\" ng-click=\"toggleShowMenu()\">" +
                ...
                "        <input style=\"margin-bottom:0px\" placeholder=\"{{i18n.ngSearchPlaceHolder}}\" type=\"text\" ng-model=\"filterText\"/>" +
                "    </div>" +
        ...
        $templateCache.put("rowTemplate.html",
            "<div style=\"cursor: {{row.cursor}};line-height: {{rowHeight}}px\" ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngCell {{col.cellClass}}\" ng-cell></div>"
        );
